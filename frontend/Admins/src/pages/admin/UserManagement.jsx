                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                    <div class="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-1 w-fit">
                        <button onClick={() => handleUser("all")}
                                class="text-xs font-semibold px-3 py-1.5 rounded-md bg-indigo-600 text-white">All
                        </button>
                        <button onClick={() => handleUser("seller")}
                                class="text-xs font-semibold px-3 py-1.5 rounded-md text-slate-600 hover:bg-slate-50">Sellers
                        </button>
                        <button onClick={() => handleUser("buyer")}
                                class="text-xs font-semibold px-3 py-1.5 rounded-md text-slate-600 hover:bg-slate-50">Buyers
                        </button>
                        <button onClick={() => handleUser("suspended")}
                                class="text-xs font-semibold px-3 py-1.5 rounded-md text-slate-600 hover:bg-slate-50">Suspended
                        </button>
                    </div>
                    <input type="text" placeholder="Search users..."
                           class="w-full sm:w-64 rounded-lg border border-slate-300 px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm">
                            ...
                            <tr class="bg-slate-50 border-b border-slate-200">
                                ...
                                <th class="text-left font-semibold text-slate-500 px-6 py-3 text-xs uppercase tracking-wide">Join
                                    Date
                                </th>
                            ...
                            {filteredData.map((item, index) => (
                                <tr key={index}>
                                    ...
                                    <td class="px-6 py-3.5"><span
                                        class="text-xs font-semibold bg-emerald-200 text-black px-2 py-0.5 rounded-full capitalize">{item.role}</span>
                                    </td>
                                    <td class="px-6 py-3.5"><span
                                        class={`text-xs font-semibold text-black px-2 py-0.5 rounded-full capitalize ${statusColor(item.status)}`}>{item.status}</span>
                                    </td>
