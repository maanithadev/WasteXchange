    ...
    return (
        <>
            ...
            <main className="flex-1 p-8 bg-slate-50 min-h-screen">
                <div className="mb-6">
                    <h1 className="text-2xl font-bold text-slate-900">Transactions &amp; Payment Oversight</h1>
                    <p className="text-sm text-slate-500 mt-1">Monitor platform-wide payment activity</p>
                </div>

                {/* <!-- Summary stat cards --> */}
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                    <div className="bg-white rounded-lg border border-slate-200 p-4">
                        <p className="text-xs font-medium text-slate-500 mb-1">Total Revenue Processed (Current Month)</p>
                        <p className="text-2xl font-bold text-slate-900">RS. {cardData.totalRevenue || 0}</p>
                    </div>
                    <div className="bg-white rounded-lg border border-slate-200 p-4">
                        <p className="text-xs font-medium text-slate-500 mb-1">Pending Payouts (Current Month)</p>
                        <p className="text-2xl font-bold text-slate-900">RS. {cardData.pendingPayouts || 0}</p>
                    </div>
                    <div className="bg-white rounded-lg border border-slate-200 p-4">
                        <p className="text-xs font-medium text-slate-500 mb-1">Disputed Transactions (Failed Current
                            Month)</p>
                        <p className="text-2xl font-bold text-red-600">RS. {cardData.disputedTransactions || 0}</p>
                    </div>
                </div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
                    <div className="flex items-center gap-1 bg-white border border-slate-200 rounded-lg p-1 w-fit">
                        ...
                        <button onClick={() => setFilter("month")}
                            className={`text-xs font-semibold px-3 py-1.5 rounded-md ${filter === 'month' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>Current
                            Month
                        </button>
                        <button onClick={() => setFilter("year")}
                            className={`text-xs font-semibold px-3 py-1.5 rounded-md ${filter === 'year' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>Current
                            Year
                        </button>
                    </div>
                </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-slate-50 border-b border-slate-200">
                                    <th className="text-left font-semibold text-slate-500 px-6 py-3 text-xs uppercase tracking-wide">Transaction
                                        ID
                                    </th>
