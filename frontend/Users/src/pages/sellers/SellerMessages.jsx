...
const SellerMessages = () => {
    ...
    useEffect(() => {
        async function checkOnlineStatus() {
            if (activeConversation && activeConversation.buyer_id) {
                try {
                    const res = await axios.get(import.meta.env.VITE_USERS_BACKEND_URL + "/api/messages/check-online-status/" + activeConversation.buyer_id);
    ...
    }, [conversationId, activeConversation?.buyer_id]);
            ...
            <main className="flex-1 bg-slate-50 h-[calc(100vh-4rem)] flex overflow-hidden">
                ...
                <div className={`${conversationId !== "" ? "hidden md:flex" : "flex"} w-full md:w-80 bg-white border-r border-slate-200 flex-col shrink-0`}>
                    <div className="p-4 border-b border-slate-200">
                        ...
                        {/* <input type="text" placeholder="Search conversations..." className="mt-3 w-full rounded-lg border border-slate-300 px-3 py-2 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500" /> */}
                    </div>
                        ...
                        {conversations.map((item, index) => (
                            <li key={index} className={`flex items-center gap-3 px-4 py-3 bg-blue-50 border-l-4 ${conversationId === item._id ? "border-blue-600" : "border-transparent"} cursor-pointer`} onClick={() => setConversationId(item._id)}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <p className="text-sm font-semibold text-slate-900 truncate">{item.buyerDetails?.company_name}</p>
                                    {/* last message */}
                    ...
                    <>
                        <div className={`flex-1 flex-col ${conversationId !== "" ? "flex" : "hidden md:flex"}`}>
                            ...
                            <div className="h-16 flex items-center gap-3 px-6 border-b border-slate-200 bg-white shrink-0">
                                ...
                                <img src="https://placehold.co/36x36" className="w-9 h-9 rounded-full object-cover" alt="EcoPlast Industries" />
                                <div>
                                    {/* company name */}
                                    <p className="text-sm font-semibold text-slate-900">{activeConversation?.buyerDetails?.company_name}</p>
                            ...
                            <div className="border-t border-slate-200 bg-white p-4 flex items-center gap-3">
                                {/* <button className="text-slate-400 hover:text-slate-600">
                                    <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" /></svg>
                                </button> */}
                                <input type="text" placeholder="Type a message..." value={typedMessage} onChange={(e) => setTypedMessage(e.target.value)} className="flex-1 rounded-full border border-slate-300 px-4 py-2.5 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500" />
                                <button onClick={sendMessage} className="bg-emerald-600 hover:bg-emerald-700 text-white rounded-full w-10 h-10 flex items-center justify-center shrink-0">
